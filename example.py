...
def find_silence(in_file):
    ...
    myprint('longuest_segment ' + str(longuest_time / 1000.0) + ' seconds', 2)
...
def sox_denoise(in_file):
    ...
    command = f'{SOX_PATH} "{in_file}" "{out_file}" noisered {SILENCE_PROFILE} 0.3'
    myprint(command, 0)
    myprint(subprocess.call(command), 1)
...
def do_actions(actions, params):
    ...
    count = 1
    for f in file_list:
        myprint(f'[{count}/{len(file_list)}] Processing : {f}')
        count += 1
